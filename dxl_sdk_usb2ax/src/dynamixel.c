...
void dxl_tx_packet()
{
    ...
    if (gbInstructionPacket[INSTRUCTION] == INST_READ)
        ...
        dxl_hal_set_timeout(gbInstructionPacket[PARAMETER + 1] + 6);
    // TODO compute reasonable timeout for SYNC_READ even in the case
    // where some servos do not answer!!!
    else
...
void dxl_rx_packet()
{
    ...
    for (i = 0; i < (gbRxGetLength - 1); i++)    // Find packet header
    {
    ...
    if (gbInstructionPacket[ID] != gbStatusPacket[ID])    // Check id pairing
    {
        gbCommStatus = COMM_RXCORRUPT;
        giBusUsing = 0;
        return;
    ...
    for (i = 0; i < (gbStatusPacket[LENGTH] + 1); i++)    // Check checksum
...
{
    while (giBusUsing)
        ; // needs to be done before touching the TX buffer as it is used until the end of RX.
